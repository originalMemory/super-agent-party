...
- [ ] 5.6 newtts 音色标签注入适配 lover：移除对 `is_memory`（角色卡）的依赖，改为 lover 人设文件存在时触发；`selectedMemoryName` 替换为 lover 角色名（搁置）
- [ ] 5.7 newVRM 外观适配 lover：对话界面 VRM 按钮硬编码 `default`，无视 newVRM 配置；需支持选中指定外观或按角色名自动关联（搁置）
