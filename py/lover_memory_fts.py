...
def lover_memory_options(settings: dict | None) -> dict[str, Any]:
    """Options from settings['loverSettings']; defaults match settings_template.json."""
    ls = (settings or {}).get("loverSettings") or {}
    minutes = ls.get("memoryIndexSyncIntervalMinutes", 10)
    try:
...
def workspace_root_from_settings(settings: dict) -> Path | None:
    """兼容旧名：等同于 ``lover_data_root()``（不再读取 ``cc_path``）。"""
