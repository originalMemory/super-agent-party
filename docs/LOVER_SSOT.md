...
- **人设 / 用户 / 元指令 / 根备忘**：相对 **CLI 工作目录**（`cwd`）下的 **`.agent/`**（如 `USER.md` 等；若与 OpenClaw 一致放在工作区根，须在实现与本文同步说明）。
- **按日日记**：**不**放在隐藏目录；采用对工作区 **用户直接可见、可浏览与手改** 的规范路径（见下「按日日记」）。
...
- **路径模式**：`memory/YYYY/MM/YYYY-MM-DD.md`（相对 `cwd`）。  
  例：`memory/2026/05/2026-05-10.md`。
- **设计意图**：**不隐藏**（非点目录、非仅程序可写路径），便于用户在资源管理器 / IDE 中 **查看与编辑**。
- **FTS**：该路径下内容 **纳入** 记忆语料索引（与 `MEMORY.md` 同为 FTS 白名单的一部分）。
...
- **语料白名单**：**`MEMORY.md`** + **`memory/YYYY/MM/*.md` 下按日文件**（及实现需包含的其它纯记忆文件，若有）。
